...
const Page = async ({ params, searchParams }: { params: Promise<{}>, searchParams: Promise<{ message: string }> }) => {
    ...
    const options = await axios.get({ raw: true, url: '/api/data/inconformidades', data: { auth_id: session.user.id }, token: session.accessToken, process: false, message: false })
    ...
    return <Authorized subject="inconformidades" action="manage">
